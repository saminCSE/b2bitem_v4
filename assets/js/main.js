...
$(document).ready(function () {
	...
	if ($(".sticky-header-2").length && $(window).width() >= 992) {
		var sticky2 = new Waypoint.Sticky({
			element: $(".sticky-header-2")[0],
			stuckClass: "fixed",
			offset: function () {
				// Calculate the offset dynamically to place sticky-header-2 under sticky-header
				var stickyHeaderHeight = $(".sticky-header").outerHeight();
				return -300 + stickyHeaderHeight;
			},
			handler: function (direction) {
		...
		// Adjust top value for sticky-header-2 on load and resize
		function adjustStickyHeader2Top() {
			var stickyHeaderHeight = $(".sticky-header").outerHeight();
			$(".sticky-header-2.fixed").css("top", stickyHeaderHeight + "px");
		}
		adjustStickyHeader2Top();
		$(window).on("resize", adjustStickyHeader2Top);
	}
