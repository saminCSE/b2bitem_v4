...
class Frontend_Controller extends CI_Controller {
    ...
    function access_check() {
        ...
        $admin_group_list = array('Seller','Buyer');

		if (in_array($admin_group, $admin_group_list)) {
            return true;
        } elseif ($this->name == 'website' || $this->name == 'product' || $this->name == 'company' || $this->name == 'category' || $this->name == 'country') {
            return true;
        } else {
            redirect('website');
        }
    ...
    function _check_access() {
        ...
        if ($username && $userid) {
            ...
            // echo '<pre>';print_r('sadffasdf');exit;

            $user = $this->websitemodel->get_logged_user();

            if (empty($user)) {

               redirect(base_url('/website/login?mt='.mt_rand(0,99999999)));

            } elseif ($this->name == 'website' && $this->method != 'logout') {
                //redirect('dashboard');
            } else {
            ...
            if ($ret) {
                ...
                if ($this->name == 'website' || $this->name == 'product' || $this->name == 'category' || $this->name == 'company' || $this->name == 'country') {
